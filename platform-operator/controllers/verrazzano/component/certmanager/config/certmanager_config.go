...
func checkRenewAllCertificates(log vzlog.VerrazzanoLogger, cli crtclient.Client, config *vzapi.ClusterIssuerComponent, isCAConfig bool) error {
...
func cleanupUnusedResources(log vzlog.VerrazzanoLogger, client crtclient.Client, issuerConfig *vzapi.ClusterIssuerComponent, isCAValue bool) error {
	defaultCANotUsed := func() bool {
		...
		return !isCAValue || issuerConfig.CA.SecretName != cmcommon.DefaultCACertificateSecretName
	}
	...
	if defaultCANotUsed() {
		...
		if err := deleteObject(client, cmcommon.DefaultCACertificateSecretName, clusterResourceNamespace, &v1.Secret{}); err != nil {
			return err
		}
	}
	return nil
...
func (c certManagerConfigComponent) createOrUpdateClusterIssuer(compContext spi.ComponentContext) error {
	...
	clusterIssuerConfig := compContext.EffectiveCR().Spec.Components.ClusterIssuer
	if clusterIssuerConfig == nil {
		return compContext.Log().ErrorfThrottledNewErr("Cluster issuer is not configured")
	}
	issuerConfig := clusterIssuerConfig.Certificate

	isCAValue, err := cmcommon.IsCAConfig(issuerConfig)
	if err != nil {
	...
	if !isCAValue {
		...
		if opResult, err = createOrUpdateAcmeResources(compContext.Log(), compContext.Client(), compContext.EffectiveCR(), clusterIssuerConfig); err != nil {
	...
	if err := cleanupUnusedResources(compContext.Log(), compContext.Client(), clusterIssuerConfig, isCAValue); err != nil {
		return err
	}
	if err := checkRenewAllCertificates(compContext.Log(), compContext.Client(), clusterIssuerConfig, isCAValue); err != nil {
...
func (c certManagerConfigComponent) deleteCertManagerIssuerResources(log vzlog.VerrazzanoLogger, client crtclient.Client, config *vzapi.ClusterIssuerComponent) error {
